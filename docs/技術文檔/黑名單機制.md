...
> ⚠️ **注意**：本文檔記錄歷史演進。目前實際使用 v0.6.0「只阻擋檔案寫入」機制。
> 詳見 [Bash白名單修復.md](Bash白名單修復.md)

## 機制演進時間線

| 版本 | 日期 | 機制 | 狀態 |
|------|------|------|------|
| v0.5.x | 2026-01-28 | 黑名單刪去法 | 已棄用 |
| **v0.6.0** | 2026-01-30 | 只阻擋檔案寫入 | **目前使用** |
...
儘管黑名單機制成功改善了使用體驗，但仍存在以下問題：
- 需要維護兩個清單（程式碼副檔名 + 核心目錄）
- 對於新的程式語言需要手動新增副檔名
- 邏輯分散於多個函式，不夠集中

**v0.6.0 進一步簡化**：只關注「是否寫入檔案」這一核心問題，不再區分檔案類型。
