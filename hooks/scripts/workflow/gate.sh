...
# 獨立測試關鍵字（執行現有測試，非測試新變更）
readonly STANDALONE_TEST_KEYWORDS="執行測試|執行 test|run tests?|run all tests?|npm test|yarn test|pnpm test|pytest|jest|vitest|mocha|cargo test|go test|rspec|dotnet test|mvn test|gradle test|測試一下|跑測試|驗證功能|check tests?|測試通過|測試失敗|閃退|crash|驗證|verify"

# 程式碼變更關鍵字（如果包含這些，則不視為獨立測試）
readonly CODE_CHANGE_KEYWORDS="變更|修改|實作|implement|修復|fix|新增|add feature|重構|refactor|更新程式|update code|寫入|write|編輯|edit|create|delete|patch|建立|刪除|修補"

# 獨立測試偵測函數
# 參數: $1 = prompt, $2 = state_valid, $3 = last_agent
# 返回: 0 = 是獨立測試, 1 = 不是獨立測試
is_standalone_test() {
    local prompt="$1"
    local state_valid="$2"
    ...
    # 條件 1: 沒有有效的 D→R→T 流程狀態
    # 如果有有效的 DEVELOPER 或 REVIEWER 狀態，表示在流程中
    if [ "$state_valid" = true ]; then
        if [ "$last_agent" = "developer" ] || [ "$last_agent" = "reviewer" ]; then
            return 1  # 在 D→R→T 流程中，不是獨立測試
        fi
    fi

    # 條件 2: prompt 包含獨立測試關鍵字
    if ! echo "$prompt" | grep -qiE "$STANDALONE_TEST_KEYWORDS"; then
        return 1  # 沒有測試關鍵字
    fi

    # 條件 3: prompt 不包含程式碼變更關鍵字
    if echo "$prompt" | grep -qiE "$CODE_CHANGE_KEYWORDS"; then
        return 1  # 包含變更關鍵字，可能是測試新變更
    fi
...
case "$SUBAGENT_TYPE" in
    ...
    tester)
        ...
        # ═══════════════════════════════════════════════════════════════
        # 獨立測試檢查（優先於 D→R→T 流程檢查）
        # ═══════════════════════════════════════════════════════════════
        if is_standalone_test "$PROMPT" "$STATE_VALID" "$LAST_AGENT"; then
            # E2E 統計：記錄合規（獨立測試）
            record_e2e_compliance "tester" "STANDALONE" "$CHANGE_ID"

            echo "" >&2
            echo "╔════════════════════════════════════════════════════════════════╗" >&2
            echo "║               🧪 TESTER 啟動（獨立測試模式）                    ║" >&2
            echo "╚════════════════════════════════════════════════════════════════╝" >&2
            echo "🔓 模式: 獨立測試（非 D→R→T 流程）" >&2
            echo "⚡ 允許直接執行現有測試" >&2
